...
const testHttp = (host, port, path = '/', timeout = 5000) => {
  return new Promise((resolve) => {
    ...
    const req = http.request(options, (res) => {
      resolve({ success: true, status: res.statusCode, message: res.statusMessage });
    });
...
const testPostgreSQL = async () => {
  ...
  try {
    const portOpen = await testPort(services.postgres.host, services.postgres.port);
    if (!portOpen) {
    ...
    try {
      ...
      if (stdout.includes('PostgreSQL')) {
        return { success: true, message: 'PostgreSQL connection successful', version: stdout.split('\\n')[2] };
      }
    } catch (err) {
      // Fall back to port test if psql is not available
      const portOpen = await testPort(services.postgres.host, services.postgres.port);
      if (portOpen) {
        return { success: true, message: 'PostgreSQL port accessible (psql not available)', note: 'Install postgresql-client for full testing' };
      }
...
const testRedis = async () => {
  ...
  try {
    ...
    try {
      const { stdout } = await execAsync(`redis-cli -h ${services.redis.host} -p ${services.redis.port} ping`, { timeout: 5000 });
      if (stdout.trim() === 'PONG') {
        return { success: true, message: 'Redis connection successful', response: stdout.trim() };
      }
    } catch (err) {
      // Fall back to port test if redis-cli is not available
      const portOpen = await testPort(services.redis.host, services.redis.port);
      if (portOpen) {
        return { success: true, message: 'Redis port accessible (redis-cli not available)', note: 'Install redis-cli for full testing' };
      }
      return { success: false, error: `redis-cli command failed: ${err.message}` };
    }
...
const testMailHog = async () => {
  ...
  try {
    // Test SMTP port
    const smtpOpen = await testPort(services.mailhog.smtp.host, services.mailhog.smtp.port);
    if (!smtpOpen) {
      return { success: false, error: 'SMTP port 1025 is not accessible' };
    }
    
    // Test Web UI
    const webResult = await testHttp(services.mailhog.web.host, services.mailhog.web.port);
    if (!webResult.success) {
      return { success: false, error: `Web UI not accessible: ${webResult.error}` };
    }
...
const testMinIO = async () => {
  ...
  try {
    // Test API port
    const apiOpen = await testPort(services.minio.api.host, services.minio.api.port);
    if (!apiOpen) {
      return { success: false, error: 'API port 9000 is not accessible' };
    }
    
    // Test Console port
    const consoleOpen = await testPort(services.minio.console.host, services.minio.console.port);
    if (!consoleOpen) {
      return { success: false, error: 'Console port 9001 is not accessible' };
    }
    
    // Test health endpoint
    const healthResult = await testHttp(services.minio.api.host, services.minio.api.port, '/minio/health/live');
    ...
    return { 
      ...
      health: healthResult.success ? 'Health check passed' : 'Health check failed'
    };
  } catch (error) {
    return { success: false, error: error.message };
  }
  ...
  for (const { name, test } of tests) {
    try {
      ...
      if (result.success) {
        ...
        if (result.serverVersion) console.log(`   Server Version: ${result.serverVersion}`);
...
export { runTests, testPostgreSQL, testRedis, testMailHog, testMinIO, testDockerDaemon };