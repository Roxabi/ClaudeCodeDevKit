...
requiredEnvVars.forEach(envVar => {
    ...
    if (!value) {
        ...
        console.log(chalk.yellow(`⚠️  ${envVar.name}: "${value}" (expected: "${envVar.expected}")`));
...
try {
    ...
    if (dbUrl) {
        ...
        console.log(chalk.gray(`   Password: ${url.password ? '[HIDDEN]' : 'NOT SET'}`));
...
if (validationErrors === 0) {
    ...
    console.log(chalk.red.bold(`❌ ${validationErrors} validation error(s) found`));
